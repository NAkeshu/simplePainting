 ...
 *  @author Kevin Wayne
 */
public final class StdDrawD implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
    ...
    private static BufferedImage tmpscreenImage,bgscreenImage;
    private static Graphics2D tmpscreen,bgscreen;

    // singleton for callbacks: avoids generation of extra .class files
    private static StdDrawD std = new StdDrawD();
    ...
    // queue队列 of typed key characters
     ...
     * This also erases the current drawing and resets the coordinate system(重设坐标系),
     * pen radius, pen color, and font back to their default values.
     * Ordinarly一丝不苟, this method is called once, at the very beginning
     * of a program.
     */
    ...
    /**
     * Sets the canvas画布 (drawing area) to be <em>width</em>-by-<em>height</em> pixels.
     * This also erases the current drawing and resets the coordinate system,
     * pen radius, pen color, and font back to their default values.
     * Ordinarly, this method is called once, at the very beginning
     * of a program.
     *
     * @param  canvasWidth the width as a number of pixels像素
    ...
    private static void init() {
        ...
        tmpscreenImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        tmpscreen = tmpscreenImage.createGraphics();

        bgscreenImage = new BufferedImage(width, height, BufferedImage.TYPE_INT_ARGB);
        bgscreen = bgscreenImage.createGraphics();
    ...
    ***************************************************************************/
    ...
    public static void clear(Color color) {
        ...
        draw();
        bgscreen.drawImage(offscreenImage,0,0,null);
    }
    ...
    ***************************************************************************/
    ...
    private static Image getImage(String filename) {
        ...
        if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
            URL url = StdDrawD.class.getResource(filename);
            if (url != null)
                icon = new ImageIcon(url);
        }
...
*/
    ...
    ***************************************************************************/
    ...
    public static void showbackground(){
        offscreen.drawImage(bgscreenImage, 0, 0, null);
        show();
    }
    public static void showsave(){
        tmpscreen.drawImage(offscreenImage,0,0,null);
    }

    public static void showload(){
        offscreen.drawImage(tmpscreenImage, 0, 0, null);
        show();
    }
    ...
    ***************************************************************************/
    ...
    public void actionPerformed(ActionEvent e) {
        FileDialog chooser = new FileDialog(StdDrawD.frame, "Use a .png or .jpg extension", FileDialog.SAVE);
        chooser.setVisible(true);
        String filename = chooser.getFile();
        if (filename != null) {
            StdDrawD.save(chooser.getDirectory() + File.separator + chooser.getFile());
        }
    }
    ...
    ***************************************************************************/
    ...
    public void mousePressed(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = StdDrawD.userX(e.getX());
            mouseY = StdDrawD.userY(e.getY());
            mousePressed = true;
            single.mousePressedCall(mouseX,mouseY);
        }
    }
    ...
    public void mouseDragged(MouseEvent e)  {//拖拽
        synchronized (mouseLock) {
            ...
            mouseX = StdDrawD.userX(e.getX());
            mouseY = StdDrawD.userY(e.getY());
            single.mouseDragCall(prex,prey,mouseX,mouseY);
        }
    }
    ...
    public void mouseMoved(MouseEvent e) {
        synchronized (mouseLock) {
            mouseX = StdDrawD.userX(e.getX());
            mouseY = StdDrawD.userY(e.getY());
            single.mouseMovedCall(mouseX,mouseY);
        }
    }
    ...
    *  as an argument. It can handle action keys
     ...
     */
    public static void main(String[] args) {
        StdDrawD.square(0.2, 0.8, 0.1);
        StdDrawD.filledSquare(0.8, 0.8, 0.2);
        StdDrawD.circle(0.8, 0.2, 0.2);

        StdDrawD.setPenColor(StdDrawD.BOOK_RED);
        StdDrawD.setPenRadius(0.02);
        StdDrawD.arc(0.8, 0.2, 0.1, 200, 45);

        // draw a blue diamond
        StdDrawD.setPenRadius();
        StdDrawD.setPenColor(StdDrawD.BOOK_BLUE);
        double[] x = { 0.1, 0.2, 0.3, 0.2 };
        double[] y = { 0.2, 0.3, 0.2, 0.1 };
        StdDrawD.filledPolygon(x, y);

        // text
        StdDrawD.setPenColor(StdDrawD.BLACK);
        StdDrawD.text(0.2, 0.5, "black text");
        StdDrawD.setPenColor(StdDrawD.WHITE);
        StdDrawD.text(0.8, 0.8, "white text");
    }
